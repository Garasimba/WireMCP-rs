...
fn run_ddos(cli: &Cli) {
    ...
    if total_syn > 100 && syn_ratio > 3.0 {
    ...
    // RST Flood
    if total_rst > 100 && pct(total_rst, total_pkts) > 30.0 {
        let msg = format!("RST Flood: {} RST packets ({:.0}%)", total_rst, pct(total_rst, total_pkts));
        println!("  >> {}", msg); alerts.push(msg);
    }
    // UDP Flood
    if udp_total > 100 && pct(udp_total, total_pkts) > 80.0 && pps > 100.0 {
        let msg = format!("UDP Flood: {:.0}% UDP at {:.0} pkt/s", pct(udp_total, total_pkts), pps);
        println!("  >> {}", msg); alerts.push(msg);
    }
    // ICMP Flood
    if icmp_total > 50 && pct(icmp_total, total_pkts) > 50.0 {
        let msg = format!("ICMP Flood: {} packets ({:.0}%)", icmp_total, pct(icmp_total, total_pkts));
        println!("  >> {}", msg); alerts.push(msg);
    }
    // Fragmentation
    ...
    if total_http_req > 0 {
        ...
        // HTTP GET Flood
        if total_gets > 100 && http_rps > 50.0 {
            let msg = format!("HTTP GET Flood: {} requests at {:.0} req/s", total_gets, total_gets as f64 / duration);
            println!("  >> {}", msg); alerts.push(msg);
        }

        // HTTP POST Flood
        if total_posts > 50 && total_posts as f64 / duration > 20.0 {
            let msg = format!("HTTP POST Flood: {} requests at {:.0} req/s", total_posts, total_posts as f64 / duration);
            println!("  >> {}", msg); alerts.push(msg);
        }
...
fn pct(n: u64, total: u64) -> f64 { if total > 0 { n as f64 / total as f64 * 100.0 } else { 0.0 } }
